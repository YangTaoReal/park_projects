...
Shader "BOXOPHOBIC/Advanced Dynamic Shaders/Standard Grass"
{
	Properties
	{
	...
	SubShader
	{
		...
		void vertexDataFunc( inout appdata_full v, out Input o )
		{
			...
			half MotionScale60_g913 = ( ADS_GlobalScale * _MotionScale );
			half MotionSpeed62_g913 = ( ADS_GlobalSpeed * _MotionSpeed );
			float mulTime90_g913 = _Time.y * MotionSpeed62_g913;
			float2 appendResult115_g905 = (float2(ase_worldPos.x , ase_worldPos.z));
			float2 panner73_g905 = ( _Time.y * ( ADS_NoiseSpeed * (-ADS_GlobalDirection).xz ) + ( appendResult115_g905 * ADS_NoiseScale ));
			float ifLocalVar94_g905 = 0;
			UNITY_BRANCH 
			if( ( ADS_NoiseTex_ON * _MotionNoise ) > 0.01 )
				ifLocalVar94_g905 = saturate( pow( abs( tex2Dlod( ADS_NoiseTex, float4( panner73_g905, 0, 0.0) ).r ) , ADS_NoiseContrast ) );
			else if( ( ADS_NoiseTex_ON * _MotionNoise ) < 0.01 )
				ifLocalVar94_g905 = 1.0;
			half MotionlAmplitude58_g913 = ( ADS_GlobalAmplitude * _MotionAmplitude * ifLocalVar94_g905 );
			half3 MotionDirection59_g913 = ( ADS_GlobalDirection + 0.0001 );
			float temp_output_25_0_g908 = _MaskAxis;
			float lerpResult24_g908 = lerp( v.texcoord3.xyz.x , v.texcoord3.xyz.y , saturate( temp_output_25_0_g908 ));
			float lerpResult21_g908 = lerp( lerpResult24_g908 , v.texcoord3.xyz.z , step( 2.0 , temp_output_25_0_g908 ));
			half THREE27_g908 = lerpResult21_g908;
			float lerpResult42_g906 = lerp( v.color.r , THREE27_g908 , _MaskType);
			float temp_output_7_0_g907 = _MaskMin;
			float lerpResult31_g906 = lerp( 0.0 , 1.0 , saturate( ( ( lerpResult42_g906 - temp_output_7_0_g907 ) / ( _MaskMax - temp_output_7_0_g907 ) ) ));
			half MotionMask137_g913 = ( _Show_MaskGeneric * lerpResult31_g906 );
			float lerpResult116_g912 = lerp( ADS_GrassSizeMin , ADS_GrassSizeMax , tex2Dlod( ADS_GrassSizeTex, float4( ( ( (ase_worldPos).xz * (ADS_GrassSizeScaleOffset).xy ) + (ADS_GrassSizeScaleOffset).zw ), 0, 0.0) ).r);
			float3 temp_cast_3 = (0.0).xxx;
			float3 ifLocalVar96_g912 = 0;
			UNITY_BRANCH 
			if( ( ADS_GrassSizeTex_ON * _GrassSize ) > 0.5 )
				ifLocalVar96_g912 = ( lerpResult116_g912 * v.texcoord3.xyz );
			else if( ( ADS_GrassSizeTex_ON * _GrassSize ) < 0.5 )
				ifLocalVar96_g912 = temp_cast_3;
			v.vertex.xyz += ( mul( unity_WorldToObject, float4( ( ( ( ( sin( ( ( ( ( ase_worldPos + (ase_worldPos).zxy ) * MotionScale60_g913 ) + mulTime90_g913 ) + ( v.color.g * 1.756 ) ) ) * MotionlAmplitude58_g913 ) + ( MotionlAmplitude58_g913 * saturate( MotionScale60_g913 ) ) ) * MotionDirection59_g913 ) * MotionMask137_g913 ) , 0.0 ) ).xyz + ifLocalVar96_g912 );
			float4 temp_cast_4 = (1.0).xxxx;
			float4 tex2DNode75_g909 = tex2Dlod( ADS_GrassTintTex, float4( ( ( (ase_worldPos).xz * (ADS_GrassTintScaleOffset).xy ) + (ADS_GrassTintScaleOffset).zw ), 0, 0.0) );
			float4 lerpResult115_g909 = lerp( ADS_GrassTintColorOne , ADS_GrassTintColorTwo , tex2DNode75_g909.r);
			float4 lerpResult121_g909 = lerp( tex2DNode75_g909 , lerpResult115_g909 , ADS_GrassTintModeColors);
			float4 lerpResult126_g909 = lerp( temp_cast_4 , ( lerpResult121_g909 * ADS_GrassTintIntensity ) , saturate( ADS_GrassTintIntensity ));
			float4 temp_cast_5 = (1.0).xxxx;
			float4 ifLocalVar96_g909 = 0;
			UNITY_BRANCH 
			if( ( ADS_GrassTintTex_ON * _GrassTint ) > 0.5 )
				ifLocalVar96_g909 = saturate( lerpResult126_g909 );
			else if( ( ADS_GrassTintTex_ON * _GrassTint ) < 0.5 )
				ifLocalVar96_g909 = temp_cast_5;
			o.vertexToFrag1077 = ifLocalVar96_g909;
		}

		void surf( Input i , inout SurfaceOutputStandard o )
		{
			float switchResult12_g911 = (((i.ASEVFace>0)?(1.0):(-1.0)));
			...
			half3 NORMAL620 = ( switchResult12_g911 * UnpackScaleNormal( tex2D( _BumpMap, MainUVs587 ), _BumpScale ) );
			o.Normal = NORMAL620;
			...
			float temp_output_1_0_g910 = ( MainTexAlpha616 * MainColorAlpha1057 );
			#if defined(_RENDERTYPE_OPAQUE)
				float staticSwitch24_g910 = 1.0;
			#elif defined(_RENDERTYPE_CUT)
				float staticSwitch24_g910 = 1.0;
			#elif defined(_RENDERTYPE_FADE)
				float staticSwitch24_g910 = 1.0;
			#elif defined(_RENDERTYPE_TRANSPARENT)
				float staticSwitch24_g910 = temp_output_1_0_g910;
			#else
				float staticSwitch24_g910 = 1.0;
			#endif
			o.Albedo = ( MainTex487 * MainColor486 * i.vertexToFrag1077 * staticSwitch24_g910 ).rgb;
			...
			#if defined(_RENDERTYPE_OPAQUE)
				float staticSwitch15_g910 = 1.0;
			#elif defined(_RENDERTYPE_CUT)
				float staticSwitch15_g910 = 1.0;
			#elif defined(_RENDERTYPE_FADE)
				float staticSwitch15_g910 = temp_output_1_0_g910;
			#elif defined(_RENDERTYPE_TRANSPARENT)
				float staticSwitch15_g910 = temp_output_1_0_g910;
			#else
				float staticSwitch15_g910 = 1.0;
			#endif
			o.Alpha = staticSwitch15_g910;
			#if defined(_RENDERTYPE_OPAQUE)
				float staticSwitch23_g910 = 1.0;
			#elif defined(_RENDERTYPE_CUT)
				float staticSwitch23_g910 = temp_output_1_0_g910;
			#elif defined(_RENDERTYPE_FADE)
				float staticSwitch23_g910 = 1.0;
			#elif defined(_RENDERTYPE_TRANSPARENT)
				float staticSwitch23_g910 = 1.0;
				...
				float staticSwitch23_g910 = 1.0;
			#endif
			clip( staticSwitch23_g910 - _Cutoff );
		}
		...
		Pass
		{
			...
			v2f vert( appdata_full v )
			{
				...
				o.customPack2.xyzw = customInputData.vertexToFrag1077;
				o.worldPos = worldPos;
			...
			) : SV_Target
			{
				...
				surfIN.vertexToFrag1077 = IN.customPack2.xyzw;
...
/*ASEBEGIN
Version=15500
1927;29;1906;1014;-1763.322;1231.222;1;True;False
...
Node;AmplifyShaderEditor.GetLocalVarNode;791;-1280,-1664;Float;False;616;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.FunctionNode;1095;-1280,-2032;Float;False;ADS Grass Tint;14;;909;b810a046229fab449ab4dfc3f9df6e7f;0;0;1;COLOR;85
...
Node;AmplifyShaderEditor.FunctionNode;1053;-1280,-1472;Float;False;ADS Mask Generic;24;;906;2cfc3815568565c4585aebb38bd7a29b;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.FunctionNode;1092;-1280,-1408;Float;False;ADS Motion Noise;11;;905;047eb809542f42d40b4b5066e22cee72;0;0;1;FLOAT;85
Node;AmplifyShaderEditor.FunctionNode;1064;2112,-896;Float;False;Switch Back Normal;-1;;911;121446c878db06f4c847f9c5afed7cfe;0;1;13;FLOAT3;0,0,0;False;1;FLOAT3;0
...
Node;AmplifyShaderEditor.VertexToFragmentNode;1077;-1088,-2032;Float;False;1;0;COLOR;0,0,0,0;False;1;COLOR;0
Node;AmplifyShaderEditor.FunctionNode;1094;-1024,-1472;Float;False;ADS Motion Global;20;;913;a8838de3869103540a427ac470da4da6;0;2;136;FLOAT;0;False;133;FLOAT;0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.FunctionNode;1093;-1024,-1344;Float;False;ADS Grass Size;17;;912;6675a46c54a0e244fb369c824eead1af;0;0;1;FLOAT3;85
...
Node;AmplifyShaderEditor.GetLocalVarNode;36;-1280,-2176;Float;False;487;0;1;COLOR;0
...
Node;AmplifyShaderEditor.FunctionNode;1073;-768,-1664;Float;False;ADS Render Type;-1;;910;fcf5ead2adb2e514895d795dcf8514b1;0;1;1;FLOAT;0;False;3;FLOAT;0;FLOAT;13;FLOAT;25
Node;AmplifyShaderEditor.GetLocalVarNode;1083;-1280,-2112;Float;False;486;0;1;COLOR;0
...
Node;AmplifyShaderEditor.GetLocalVarNode;752;-1280,-1824;Float;False;748;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;550;-1280,-2560;Half;False;Property;_SrcBlend;_SrcBlend;31;1;[HideInInspector];Create;True;0;0;True;0;1;1;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;925;-384,-2560;Half;False;Property;_ZWrite;_ZWrite;30;1;[HideInInspector];Create;True;2;Off;0;On;1;0;True;0;1;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;1074;-512,-2176;Float;False;4;4;0;COLOR;0,0,0,0;False;1;COLOR;0,0,0,0;False;2;COLOR;0,0,0,0;False;3;FLOAT;0;False;1;COLOR;0
Node;AmplifyShaderEditor.RangedFloatNode;553;-1120,-2560;Half;False;Property;_DstBlend;_DstBlend;32;1;[HideInInspector];Create;True;0;0;True;0;10;10;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.StandardSurfaceOutputNode;0;-256,-2048;Float;False;True;2;Float;ADSShaderGUI;300;0;Standard;BOXOPHOBIC/Advanced Dynamic Shaders/Standard Grass;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;True;False;False;False;Off;0;True;925;0;False;-1;False;0;False;-1;0;False;-1;False;0;Custom;0;True;True;0;True;Opaque;;Geometry;All;False;True;True;False;True;True;True;True;True;True;True;True;True;True;True;True;True;0;False;-1;False;0;False;-1;255;False;-1;255;False;-1;0;False;-1;0;False;-1;0;False;-1;0;False;-1;0;False;-1;0;False;-1;0;False;-1;0;False;-1;False;2;15;10;25;False;0.5;True;1;5;True;550;10;True;553;0;1;False;550;10;False;553;0;True;-1;0;False;-1;0;False;0;0,0,0,0;VertexOffset;True;False;Cylindrical;False;Relative;300;Standard;-1;-1;-1;-1;0;False;0;0;True;743;-1;0;True;862;3;Include;VS_indirect.cginc;Pragma;instancing_options procedural:setup;Pragma;multi_compile GPU_FRUSTUM_ON __;0;0;16;0;FLOAT3;0,0,0;False;1;FLOAT3;0,0,0;False;2;FLOAT3;0,0,0;False;3;FLOAT;0;False;4;FLOAT;0;False;5;FLOAT;0;False;6;FLOAT3;0,0,0;False;7;FLOAT3;0,0,0;False;8;FLOAT;0;False;9;FLOAT;0;False;10;FLOAT;0;False;13;FLOAT3;0,0,0;False;11;FLOAT3;0,0,0;False;12;FLOAT3;0,0,0;False;14;FLOAT4;0,0,0,0;False;15;FLOAT3;0,0,0;False;0
...
WireConnection;1077;0;1095;85
WireConnection;1094;136;1053;0
WireConnection;1094;133;1092;85
...
WireConnection;1082;0;1094;0
WireConnection;1082;1;1093;85
...
ASEEND*/
//CHKSM=A63DCF129820DC6C876CE8513E9185D2D9BAFC4D