...
namespace PixelCrushers.DialogueSystem
{
    ...
    public class StandardUISubtitlePanel : UIPanel
    {
        ...
        {
            //Debug.LogError("排序前：" + formattedText.text);
            string t = UITools.GetUIFormattedText(formattedText);
            if(Localization.language == "ar" )
            {
                t = ArabicFixer.Fix(string.Format(t), true, false);
                textField.text = previousText + t;
            }
            else
            {
            ...
            //Debug.LogError("排序后：" + ArabicFixer.Fix(string.Format(formattedText.text), true, false));
