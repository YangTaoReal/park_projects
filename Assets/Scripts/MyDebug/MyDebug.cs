...
public class MyDebug : MonoBehaviour
{
	...
	/// 是否进入战斗
	/// </summary>
	public static bool isEnterFight = false;
	...
	{
		//if (isShowLog) {
        if (Directory.Exists(DataPath) == false)//如果不存在就创建file文件夹
        {
            Directory.CreateDirectory(DataPath);
        }
			//Application.persistentDataPath Unity中只有这个路径是既可以读也可以写的。
			outpath = DataPath +"/" + Application.productName + DateTime.Now.ToLocalTime ().ToString ("(HH:mm:ss:fff)") + "_" + "outLog.txt";
			//每次启动客户端删除之前保存的Log
			Application.logMessageReceived += HandleLog;
			if (System.IO.File.Exists (outpath)) {
				System.IO.File.Delete (outpath);
			}
			MyDebug.Log ("输出日志地址："+outpath);
		//}
	}
    ...
    public static string DataPath
    {
        get
        {
            ...
            {
                return Application.persistentDataPath + "/" + Application.productName + "Logs";
            }
	...
	private void OnGUI ()
	{
		...
		if (!isShowLog) {
			if (Input.GetMouseButtonUp (1)) {
				if (Time.time - lastRightClickTime < 0.1f)
					return;
		...
		if(GUI.Button(new Rect(300, 0, 80, 60), strtext)){
			//Debug.Log(UILabel.logStr);
